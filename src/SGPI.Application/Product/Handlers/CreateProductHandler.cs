    ...
    : IRequestHandler<CreateProductCommand, Guid>
{
    ...
    {
        var productCode = productCodeGenerator.GenerateProductCode(request.Name, request.Type,
            DateOnly.FromDateTime(request.MaturityDate));
        var financialProduct = FinancialProduct.Create(request.Name, request.Type, request.Value, request.MaturityDate,
            request.InterestRate, productCode);
